...
timeframe = st.sidebar.selectbox("Select Timeframe (Expiry):", ["1m", "5m", "15m", "1h"], index=0)
...
try:
    ...
    if df.empty or len(df) < 30:
        st.warning("🔄 Fetching live market candles... Please wait a few seconds.")
    else:
        ...
        pattern = "Consolidation / No Pattern"
        confidence = "50%"
        ...
        if candle_range > 0 and lower_wick > (2 * body) and current_price <= (support * 1.001):
            ...
            confidence = "90%"
        elif is_bullish and (prev['Close'] < prev['Open']) and (latest['Close'] > prev['Open']) and rsi_val < 60:
            pattern = "Strong Bullish Engulfing 🔥"
            signal = "CALL (BUY) 🟢 UP ⬆️"
            css_class = "big-signal-buy"
            confidence = "85%"
        elif is_bearish and (prev['Open'] < prev['Close']) and (latest['Close'] < prev['Open']) and rsi_val > 40:
        ...
        with col_t2:
            st.metric("⏱️ Next Candle Timer", f"{seconds_left} Sec Left")
    ...
    st.error(f"Waiting for Live Market Connection... ({e})")
